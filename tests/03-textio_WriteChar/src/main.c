...
int main(void)
{
	...
	/**
	 * Test: 02
	 * Write a character at the start of the buffer.
	*/
	textio_WriteChar(buffer, 'Z');
	dbg_sprintf(dbgout, "Buffer = ");
	autotest_prints(buffer, 10);
	dbg_sprintf(dbgout, "\n");
	if (!autotest_streq("ZBCD\0\0\0\0\0\0", buffer, 10))
	{
		dbg_sprintf(dbgout, "Test: 02: textio_WriteChar failed.\n");
		return 1;
	};
